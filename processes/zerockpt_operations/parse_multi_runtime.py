...
def parse_multi_runtime(input_paths: List[InputPath], output_path: Path):
    """解析运行日志文件
    """
    data = []
        ...
        for line in lines:
            ...
            if '[Rank 0] Step' in line:
                ...
                if 'Loss' in line:
                    ...
                    data.append([input_path.original_path, step_str, time_str, forward_str, backward_str, update_str, loss_str])

    # 保存到csv文件
    df = pd.DataFrame(data, columns=['file_path', 'step', 'time', 'forward', 'backward', 'update', 'loss'])
